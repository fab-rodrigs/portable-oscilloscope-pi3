...
// Definição do pino de Acoplamento (ex: PTA12)
//#define AC_DC_GPIO GPIOA
//#define AC_DC_PORT PORTA
//#define AC_DC_PIN 12U
...
void adc_init(void) {
    ...
    // --- ETAPA 1: Calibração (lenta) ---
    cfg.clockSource = kADC16_ClockSourceAlt1;
    cfg.clockDivider = kADC16_ClockDivider8;  // 7.5 MHz (seguro para calibração)
    cfg.resolution = kADC16_ResolutionSE10Bit;
    cfg.enableHighSpeed = false;
    ADC16_Init(ADC1_PERIPHERAL, &cfg);

    if (ADC16_DoAutoCalibration(ADC1_PERIPHERAL) != kStatus_Success) {
        while(1);
    }

    // --- ETAPA 2: Alta Velocidade (ADCK = 60 MHz) ---
    ADC16_GetDefaultConfig(&cfg);

    cfg.clockSource = kADC16_ClockSourceAlt1;     // Bus Clock = 60 MHz
    cfg.clockDivider = kADC16_ClockDivider4;      // ADCK = 60/4 = 15MHz
    cfg.resolution = kADC16_ResolutionSE10Bit;
    cfg.enableHighSpeed = true;                   // ADHSC = 1
    cfg.enableContinuousConversion = true;
    cfg.enableLowPower = false;

    ADC16_Init(ADC1_PERIPHERAL, &cfg);

    // --- Trigger por software (contínuo) ---
...
static void touchpad_read_cb(lv_indev_drv_t * drv, lv_indev_data_t * data) {
    ...
    if (p.z > 10) {
        ...
        data->point.y = map(p.y, Y_MAX_HW, Y_MIN_HW, 0, 239); // Y é invertido
...
static void chart_press_event_handler(lv_event_t * e) {
    ...
    if (point.x < 160) {
        ...
        int32_t centered_value = map(chart_y, 0, 239,
                                     scales[current_scale_index].max,
                                     scales[current_scale_index].min);

        // Converte para valor ADC bruto com offset
        g_trigger_level = centered_value + (512 + g_vertical_offset);
...
void UART0_Init_115200_PTC17_PTC16(void)
{
    /* 1. Habilita o clock do PORTC e da UART0 */
    CLOCK_EnableClock(kCLOCK_PortB);
    CLOCK_EnableClock(kCLOCK_Uart0);

    /* 2. Configura os pinos PTC17 e PTC16 como UART0_TX / UART0_RX (MUX = 3) */
    PORT_SetPinMux(PORTB, 16U, kPORT_MuxAlt3);   // PTB16 = UART2_RX
    PORT_SetPinMux(PORTB, 17U, kPORT_MuxAlt3);   // PTB17 = UART2_TX

    /* 3. Configuração da UART */
    uart_config_t config;
    UART_GetDefaultConfig(&config);

    config.baudRate_Bps = 1000000;   // ou 9600, 38400, 921600, etc.
    config.enableTx     = true;
    config.enableRx     = false;

    /* 4. Inicializa a UART0 usando o clock do núcleo (120 MHz na FRDM-K64F padrão) */
...
int main(void) {
    ...
    KSDK_GPIO_Init();
    UART0_Init_115200_PTC17_PTC16();

    // Teste imediato (opcional)
    const char *msg = "UART0 agora em PTB17 (TX) e PTB16 (RX) - 115200 8N1\r\n";
    UART_WriteBlocking(UART0, (const uint8_t *)msg, strlen(msg));
    ...
    // --- MODIFICAÇÃO: Habilitar o Contador de Ciclos DWT ---
    // (fsl_common.h já deve incluir os headers do core)
    CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk; // Habilita a "Trace"
    DWT->CTRL |= DWT_CTRL_CYCCNTENA_Msk; // Habilita o contador de ciclos
    DWT->CYCCNT = 0; // Zera o contador
    last_cyccnt = DWT->CYCCNT; // Armazena o ponto de partida
    // ------------------------------------------------------
    ...
    while (1) {
        ...
        if (data_ready_flag) {
            ...
            for (int i = 1; i < DMA_BUFFER_SIZE; i++) {
                ...
                // --- APLICA O MESMO OFFSET QUE É USADO NO DESENHO ---
                int32_t last_centered = last_raw - (512 + g_vertical_offset);
                int32_t curr_centered = curr_raw - (512 + g_vertical_offset);

                // Nível de trigger também é deslocado pelo offset (centralizado em 0)
            ...
            // --- DEBUG: Medição real via DWT ---
            uint32_t fs_khz = 0;
            if (g_cycle_count > 0) {
                fs_khz = (120000ULL * 1000ULL) / g_cycle_count;
            }
            // --------------------------------------------------------
            ...
            sprintf(info_text, "CH1\nVpp: %ld.%02ldV\nVmed: %ld.%02ldV\nCYC: %lu\nFs: %lu kHz\nFreq: %lu Hz",
                    vpp_int, vpp_frac,
                    vmed_int, vmed_frac,
                    g_cycle_count, // <-- Mostra o valor bruto de ciclos
					fs_khz,
...
void DMA0_IRQHandler(void) {
    ...
    // --- MODIFICAÇÃO: MEDIÇÃO DE TEMPO NA ISR ---
    uint32_t now_cyccnt = DWT->CYCCNT;
    g_cycle_count = now_cyccnt - last_cyccnt; // Mede o tempo exato para 100 amostras
    last_cyccnt = now_cyccnt; // Salva o tempo para a próxima interrupção
    // ------------------------------------------

    // Inverte buffers
    if (active_dma_buffer == dma_buffer_ping) {
        active_dma_buffer = dma_buffer_pong;
        ready_dma_buffer = dma_buffer_ping;
    } else {
        active_dma_buffer = dma_buffer_ping;
        ready_dma_buffer = dma_buffer_pong;
    }

    data_ready_flag = true;
    DMA0->TCD[0].DADDR = (uint32_t)active_dma_buffer;
    DMA0->SERQ = DMA_SERQ_SERQ(0); // Rearma canal
}
